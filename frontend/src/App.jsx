...
const API_UPI = import.meta.env.VITE_API_UPI || "http://localhost:3000";
...
//Calls backend and gets data
  const getStudents = async () => {
    try {
      ...
      const [studentsRes, attendanceRes] = await Promise.all([
       ...
       fetch(`${API_UPI}/students`),
        //fetch("http://localhost:3000/attendance/today")
        fetch(`${API_UPI}/attendance`),
      ]);
  ...
  const resetAttendance = async () => {
    try {
      ...
      const response = await fetch(`${API_UPI}/attendance/today`, {
        method: "DELETE"
      });
